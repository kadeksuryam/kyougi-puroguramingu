...
func f(x *big.Int) *big.Int {
	...
	b := new(big.Int).Mul(x, big.NewInt(100))
	c := new(big.Int).Div(x, big.NewInt(3))

	ans = ans.Add(ans.Add(a, b), c)
	return ans
}
...
func main() {
	...
	in := readStrings()[0]
	target, _ := new(big.Int).SetString(in, 10)
	...
	for left <= right {
		...
		curr := f(big.NewInt(int64(mid)))

		if curr.Cmp(target) == 0 {
			writeLine(mid)
			return
		} else if curr.Cmp(target) == -1 {
			left = mid + 1
		} else {
			right = mid - 1
		}
