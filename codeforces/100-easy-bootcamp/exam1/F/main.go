...
func main() {
	...
	var freqPair []pair
	for i := 0; i < 26; i++ {
		freqPair = append(freqPair, pair{rune(i + 'A'), freq[i]})
	}

	sort.Slice(freqPair, func(i, j int) bool {
		return freqPair[i].cnt > freqPair[j].cnt
	})

	ans := 0
	for i := 0; i < 26; i++ {
		if k >= freqPair[i].cnt {
			ans += freqPair[i].cnt * freqPair[i].cnt
			k -= freqPair[i].cnt
		} else {
			ans += k * k
			break
