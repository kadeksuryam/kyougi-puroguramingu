...
func main() {
	...
	mnVit = make(map[string]int)
	for i := 0; i < n; i++ {
		in := strings.Split(readLine(), " ")
		c, _ := strconv.Atoi(in[0])
		v := in[1]

		if v == "AB" || v == "BA" {
			if val, ok := mnVit["AB"]; ok {
				mnVit["AB"] = min(val, c)
			} else {
				mnVit["AB"] = c
			}
		} else if v == "AC" || v == "CA" {
			if val, ok := mnVit["AC"]; ok {
				mnVit["AC"] = min(val, c)
			} else {
				mnVit["AC"] = c
			}
		} else if v == "BC" || v == "CB" {
			if val, ok := mnVit["BC"]; ok {
				mnVit["BC"] = min(val, c)
			} else {
				mnVit["BC"] = c
			}
		} else if len(v) == 3 {
			if val, ok := mnVit["ABC"]; ok {
				mnVit["ABC"] = min(val, c)
			} else {
				mnVit["ABC"] = c
			}
		} else {
			if val, ok := mnVit[v]; ok {
				mnVit[v] = min(val, c)
			} else {
				mnVit[v] = c
			}
	...
	if isKeyExist("A") && isKeyExist("B") && isKeyExist("C") {
		ans = min(ans, mnVit["A"]+mnVit["B"]+mnVit["C"])
	}
	if isKeyExist("AB") && isKeyExist("C") {
		ans = min(ans, mnVit["AB"]+mnVit["C"])
	}
	if isKeyExist("AC") && isKeyExist("B") {
		ans = min(ans, mnVit["AC"]+mnVit["B"])
	}
	if isKeyExist("BC") && isKeyExist("A") {
		ans = min(ans, mnVit["BC"]+mnVit["A"])
	}
	if isKeyExist("AB") && isKeyExist("AC") {
		ans = min(ans, mnVit["AB"]+mnVit["AC"])
	}
	if isKeyExist("AB") && isKeyExist("BC") {
		ans = min(ans, mnVit["AB"]+mnVit["BC"])
	}
	if isKeyExist("AC") && isKeyExist("BC") {
		ans = min(ans, mnVit["AC"]+mnVit["BC"])
	}
	if isKeyExist("ABC") {
		ans = min(ans, mnVit["ABC"])
	}
