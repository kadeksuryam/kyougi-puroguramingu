...
func segmentedSieve(a, b int) int {
	lim := int(math.Sqrt(float64(b)))
	mark := make([]bool, lim+1)
	for i := 0; i <= lim; i++ {
		mark[i] = false
	}
	var primes []int
	...
	for _, prime := range primes {
		for j := max(prime*prime, ((a+prime-1)/prime)*prime); j <= b; j += prime {
			isPrime[j-a] = false
		}
	}
	if a == 1 {
