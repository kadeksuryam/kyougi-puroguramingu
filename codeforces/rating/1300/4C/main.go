...
func main() {
	...
	for n != 0 {
		...
		if !ok {
			...
			for key := range occ {
				splitKey := strings.Split(key, s)

				// writeLine(len(splitKey))
				if len(splitKey) == 2 {
					tmp, _ := strconv.Atoi(splitKey[1])
					curr = max(curr, tmp)
				}
			}

			newKey := strings.Builder{}
			newKey.WriteString(s)
			newKey.WriteString(fmt.Sprint(curr + 1))

			occ[newKey.String()] = true

			writeLine(newKey.String())
		}
		n--
	}
}
